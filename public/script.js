...
function renderGrid(){
  ...
  // Helper: pick up to N items, in priority order, without duplicates.
  const pickUpTo = (max, pools, excludeIds = new Set()) => {
    const out = [];
  ...
  const hasTag = (p, tag) => (p.tags||[]).some(t => String(t).toLowerCase() === String(tag).toLowerCase());
  const hasTagLike = (p, part) => (p.tags||[]).some(t => String(t).toLowerCase().includes(String(part).toLowerCase()));

  // Featured rail: best sellers first, then fill with curated/expensive items.
  const bestSellers = (CATALOG || []).filter(p => hasTagLike(p, "best"));
  const highTicket = (CATALOG || []).slice().sort((a,b)=>Number(b.price||0)-Number(a.price||0));
  const featuredList = pickUpTo(3, [bestSellers, (CATALOG || []), highTicket]);
  ...
  // New rail: rotate via limited-drop picks, then explicit "new", then fill.
  const dropIds = Object.keys(dropMap || {});
  const dropProducts = dropIds.map(id => getProduct(id)).filter(Boolean);
  const explicitNew = (CATALOG || []).filter(p => hasTag(p, "new"));
  const limitedLike = (CATALOG || []).filter(p => hasTag(p, "limited") || hasTag(p, "drop") || hasTagLike(p, "limited"));
  const finalNew = pickUpTo(3, [dropProducts, explicitNew, limitedLike, (CATALOG || [])], featuredIds);
